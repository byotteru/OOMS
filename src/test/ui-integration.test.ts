...
describe("UI統合テスト", () => {
  ...
  it("スタッフ削除後のリスト取得が正しく動作する", () => {
    ...
    dbManager.deleteUser(staffToDelete.id);
    console.log("削除対象スタッフ:", staffToDelete.name);

    // 削除後のアクティブスタッフ数を確認
    activeUsers = dbManager.getUsers();
  ...
  it("WeeklyReportPageで使用する週次データの取得ができる", () => {
    // 今週の月曜日を計算
    ...
    // 週次レポートの取得をテスト
    try {
      const weeklyReport = dbManager.getWeeklyReport(mondayStr);
      console.log(
        "週次レポート取得成功:",
